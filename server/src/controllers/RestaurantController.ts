...
export default class RestautantController implements Crud {

    async create(request: Request, response: Response) {
        const { name, address, food_type } = request.body;

        const isAnyUndefined = Citi.areValuesUndefined(name, address, food_type);
        if (isAnyUndefined) return response.status(400).send();

        const newRestaurant = { name, address, food_type };
    ...
    async update(request: Request, response: Response) {
        ...
        const isAnyUndefined = Citi.areValuesUndefined(name, address, food_type, id);
        if (isAnyUndefined) return response.status(400).send();

        const restaurantWithUpdatedValues = { name, address, food_type };
...
//pq nao checa valores indefinidos na delete(), e pq o request na get()